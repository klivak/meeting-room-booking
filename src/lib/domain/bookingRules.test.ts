...
const kyiv = (iso: string) =>
  DateTime.fromISO(iso, { zone: OFFICE_TZ }).toJSDate();
...
describe("working hours", () => {
  it("accepts a booking inside the office day", () => {
    expect(
      codesFor(kyiv("2026-03-10T09:00"), kyiv("2026-03-10T09:30")),
    ).toEqual([]);
  });

  it("accepts a booking that ends exactly at closing time", () => {
    expect(
      codesFor(kyiv("2026-03-10T18:30"), kyiv("2026-03-10T19:00")),
    ).toEqual([]);
  });

  it("rejects a booking that starts before opening time", () => {
    expect(
      codesFor(kyiv("2026-03-10T08:30"), kyiv("2026-03-10T09:30")),
    ).toContain("OUTSIDE_WORKING_HOURS");
  });

  it("rejects a booking that runs past closing time", () => {
    expect(
      codesFor(kyiv("2026-03-10T18:45"), kyiv("2026-03-10T19:15")),
    ).toContain("OUTSIDE_WORKING_HOURS");
  });

  it("rejects a booking that crosses midnight", () => {
    expect(
      codesFor(kyiv("2026-03-10T23:30"), kyiv("2026-03-11T00:30")),
    ).toContain("OUTSIDE_WORKING_HOURS");
  });
});
...
describe("daylight saving time in Europe/Kyiv", () => {
  it("accepts 09:00-09:30 both in winter and in summer", () => {
    expect(
      codesFor(kyiv("2026-02-10T09:00"), kyiv("2026-02-10T09:30")),
    ).toEqual([]);
    expect(
      codesFor(kyiv("2026-07-10T09:00"), kyiv("2026-07-10T09:30")),
    ).toEqual([]);
  });

  it("really is a different UTC hour on those two dates", () => {
    expect(kyiv("2026-02-10T09:00").toISOString()).toBe(
      "2026-02-10T07:00:00.000Z",
    );
    expect(kyiv("2026-07-10T09:00").toISOString()).toBe(
  ...
  it("still rejects 08:30 office time in summer, even though it is 05:30 UTC", () => {
    expect(
      codesFor(kyiv("2026-07-10T08:30"), kyiv("2026-07-10T09:30")),
    ).toContain("OUTSIDE_WORKING_HOURS");
  });
});
...
describe("granularity", () => {
  it("rejects a start off the 30-minute grid", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:15"), kyiv("2026-03-10T11:15")),
    ).toContain("TIME_NOT_ALIGNED");
  });

  it("rejects stray seconds", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:00:30"), kyiv("2026-03-10T11:00")),
    ).toContain("TIME_NOT_ALIGNED");
  });

  it("accepts both half-hour marks", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:30"), kyiv("2026-03-10T11:00")),
    ).toEqual([]);
  });
});
...
describe("duration", () => {
  it("rejects anything shorter than 30 minutes", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:00"), kyiv("2026-03-10T10:25")),
    ).toContain("DURATION_INVALID");
  });

  it("accepts exactly 4 hours", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:00"), kyiv("2026-03-10T14:00")),
    ).toEqual([]);
  });

  it("rejects 4 hours and a half", () => {
    expect(
      codesFor(kyiv("2026-03-10T10:00"), kyiv("2026-03-10T14:30")),
    ).toContain("DURATION_INVALID");
  });
});
...
describe("interval sanity", () => {
  it("rejects an end that is not after the start", () => {
    expect(
      codesFor(kyiv("2026-03-10T11:00"), kyiv("2026-03-10T10:00")),
    ).toEqual(["END_BEFORE_START"]);
    expect(
      codesFor(kyiv("2026-03-10T10:00"), kyiv("2026-03-10T10:00")),
    ).toEqual(["END_BEFORE_START"]);
  });
});
...
describe("future only", () => {
  it("rejects a booking that starts exactly now", () => {
    ...
    expect(codesFor(start, kyiv("2026-03-10T11:00"), start)).toContain(
      "TIME_IN_PAST",
    );
  });
  ...
  it("accepts a booking that starts a minute later", () => {
    ...
    expect(
      codesFor(kyiv("2026-03-10T10:00"), kyiv("2026-03-10T11:00"), now),
    ).toEqual([]);
  });
