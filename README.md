...
`npm test` — 96 юніт-тестів і 48 інтеграційних.
...
Скасувати можна окреме входження або всю серію: діалог пропонує «Лише це» і «Всю серію», а серія чіпає лише ті входження, що ще не відбулися — скасування не переписує минуле.
...
Якщо одразу після вашого бронювання кімнату займає хтось інший, за `NOTIFY_BEFORE_MINUTES` хвилин до кінця в хедері з'являється дзвіночок із лічильником і разовий toast. Якщо наступний слот вільний, попереджати нема про що — сповіщення не створюється.
...
Невдалі спроби входу рахуються за парою «адреса + джерело»: після 10 за 5 хвилин застосунок відповідає `429 TOO_MANY_ATTEMPTS`. Успішні входи не рахуються, тож звичайне користування ліміту не бачить, а блокування одного акаунта не зачіпає інших. Повторне надсилання листа підтвердження обмежене окремо.

Заголовки: `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy`; версія фреймворка не оголошується. CSP свідомо не додано — Next використовує інлайнові скрипти, і робити це правильно треба через nonce, що виходить за межі задачі.
